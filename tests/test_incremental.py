...
CHUNKS = [
    ...
    {"id": "c", "title": "내 친구의 집은 어디인가", "text": "1996년 개봉한 영화."},
    {
        "id": "d",
        "title": "Athens",
        "text": "The capital city of Greece, on the Attic peninsula.",
    },
...
def _assert_same_index(a, b):
    x, y = a.vector._lexical_index(), b.vector._lexical_index()
...
def test_empty_feedback_scores_like_no_feedback():
    ...
    q = "그리스의 수도"
    assert [c.id for c, _ in warm.retrieve(q, limit=5)] == [
        c.id for c, _ in cold.retrieve(q, limit=5)
    ]
...
def test_interleaved_remember_and_forget_at_every_step():
    ...
    for op, (q, ids) in script:
        if op == "remember":
            ...
            of.forget(q, ids)
            state = (
                _forgotten(state, q, ids[0])
                if len(ids) == 1
                else [
                    (sq, [d for d in sids if not (sq == q and d in ids)])
                    for sq, sids in state
                ]
            )
